...
class WebsiteRequests:
    ...
    def iss_nearby(self):
        """
        Uses http://open-notify.org/Open-Notify-API/ISS-Location-Now/
    ...
    def weather_is_okay(self):
        """
        Uses https://openweathermap.org/api (Current Weather Data)
        """
        current_weather = requests.get(url=f"http://api.openweathermap.org/data/2.5/weather?lat={self.lat}&"
    ...
    def get_distance(self, iss_lat, iss_lon):
        """
        Calculates the distance between you and the ISS.
        """
